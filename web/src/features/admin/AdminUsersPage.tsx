...
function UserAdminRow({ user, libraryIds, libraryNames }: { user: AdminUser; libraryIds: string[]; libraryNames: Map<string, string> }) {
  ...
  const [username, setUsername] = useState(user.usernameNormalized);
  const [displayNameInput, setDisplayNameInput] = useState(user.displayName || user.usernameNormalized);
  ...
  const update = useMutation({ mutationFn: (input: Record<string, unknown>) => api.updateAdminUser(user.id, input), onSuccess: ({ user: updatedUser }) => { setUsername(updatedUser.usernameNormalized); setDisplayNameInput(updatedUser.displayName || updatedUser.usernameNormalized); setNewPassword(""); void queryClient.invalidateQueries({ queryKey: queryKeys.adminUsers }); } });
  ...
  return <article className={user.isDisabled ? "lux-admin-user-row is-disabled" : "lux-admin-user-row"}>
    <div className="lux-admin-user-summary"><span className="lux-admin-user-avatar"><UserRound size={18} /></span><div><strong>{displayName}</strong><small>{user.usernameNormalized}{user.canManageServer ? " · 管理员" : ""}</small></div></div>
    <div className="lux-admin-user-badges"><span className={user.isDisabled ? "lux-user-badge is-warn" : "lux-user-badge is-ok"}>{user.isDisabled ? "已禁用" : "正常"}</span><span className="lux-user-badge">{user.canRemoteAccess ? "远程" : "仅本地"}</span></div>
    <div className="lux-admin-user-actions"><button className="lux-button lux-button-secondary" type="button" aria-label={`${expanded ? "收起" : "管理"}${displayName}账户`} onClick={() => setExpanded((value) => !value)}>{expanded ? <X size={15} /> : <ShieldCheck size={15} />} {expanded ? "收起" : "管理账户"}</button><button className="lux-icon-button lux-icon-button-small" type="button" aria-label={`保存 ${displayName} 权限`} onClick={() => update.mutate({ canManageServer: !user.canManageServer })} title="切换服务器管理权限"><ShieldCheck size={15} /></button><button className="lux-icon-button lux-icon-button-small lux-danger-icon lux-admin-delete-user" type="button" aria-label={`删除${displayName}账户`} title="删除账户（禁用登录）" onClick={confirmDisable} disabled={disable.isPending}><Trash2 size={15} /></button></div>
    {disable.error ? <p className="lux-error-copy lux-admin-user-action-error" role="alert">删除失败：{disable.error.message}</p> : null}
    {expanded ? <div className="lux-admin-user-detail"><div className="lux-admin-user-identity"><div className="lux-admin-subpanel-heading"><strong>账户信息</strong><span>用户名用于登录，昵称用于界面展示</span></div><div className="lux-admin-user-identity-fields"><label>用户名<input aria-label={`${displayName}的用户名`} value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="off" maxLength={128} /></label><label>昵称<input aria-label={`${displayName}的昵称`} value={displayNameInput} onChange={(event) => setDisplayNameInput(event.target.value)} maxLength={128} /></label><button className="lux-button lux-button-secondary" type="button" aria-label={`保存${displayName}账户信息`} disabled={!username.trim() || update.isPending} onClick={() => update.mutate({ username, displayName: displayNameInput })}><Save size={15} />{update.isPending ? "保存中…" : "保存账户信息"}</button></div></div><div className="lux-admin-permission-grid"><PermissionToggle label="服务器管理" checked={user.canManageServer} onChange={(checked) => update.mutate({ canManageServer: checked })} /><PermissionToggle label="远程访问" checked={user.canRemoteAccess} onChange={(checked) => update.mutate({ canRemoteAccess: checked })} /><PermissionToggle label="允许下载" checked={user.canDownload} onChange={(checked) => update.mutate({ canDownload: checked })} /><PermissionToggle label="账户启用" checked={!user.isDisabled} onChange={(checked) => update.mutate({ isDisabled: !checked })} /></div><div className="lux-admin-access-block"><div className="lux-admin-subpanel-heading"><strong>媒体库访问</strong><span>选择用户可见的库</span></div>{access.isPending ? <p className="lux-admin-muted">正在读取…</p> : libraryIds.length === 0 ? <p className="lux-admin-muted">暂无媒体库。</p> : <div className="lux-admin-library-access">{libraryIds.map((libraryId) => <label key={libraryId}><input type="checkbox" checked={enabledLibraryIds.has(libraryId)} onChange={(event) => setAccess.mutate({ libraryId, canView: event.target.checked })} /><span>{libraryNames.get(libraryId) ?? libraryId}</span></label>)}</div>}</div><div className="lux-admin-password-row"><label>设置新密码<input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" placeholder="留空不修改" /></label><button className="lux-button lux-button-secondary" type="button" disabled={!newPassword || update.isPending} onClick={() => update.mutate({ password: newPassword })}><KeyRound size={15} /> 修改密码</button></div>{update.error ? <p className="lux-error-copy">{update.error.message}</p> : null}</div> : null}
  </article>;
}
